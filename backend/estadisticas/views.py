...
class EstadisticasView(APIView):
    ...
    def get(self, request):
        ...
        # El total real de cada pedido (items + envío - descuento) es la misma cuenta
        # que Pedido.calcular_total(), pero acá la hacemos a mano sobre filas livianas
        # (values()) en vez de instanciar objetos completos de Pedido/DetallePedido con
        # prefetch_related — con años de historial, traer y armar esos objetos completos
        # es lo que hace lenta a "General" (que no tiene rango de fecha y por eso no se
        # puede acotar). Así, "General" crece mucho más lento con el tiempo, y el
        # resultado es matemáticamente idéntico al de antes.
        ...
        for fila in DetallePedido.objects.filter(pedido__in=pedidos_validos).values('id', 'pedido_id', 'precio_unitario', 'cantidad'):
            subtotal_por_pedido[fila['pedido_id']] += fila['cantidad'] * fila['precio_unitario']
...
class CobranzasView(APIView):
    ...
    def get(self, request):
        ...
        pedidos_activos = Pedido.objects.filter(confirmado=True).exclude(estado='cancelado').prefetch_related('items', 'pagos')
        pedidos_pendientes = []
