const CLAVE_LOCAL_STORAGE = 'hierbas_config_impresion';
...
function construirBloqueTicketCocina(pedido) {
  const entregaTexto = pedido.tipo_entrega === 'envio'
    ? `🚚 Envío${pedido.localidad_nombre ? ` — ${escapeHtml(pedido.localidad_nombre)}` : ''}`
    : '🏬 Retiro en local';

  const filasItems = pedido.items
    .map((item) => `<div class="ticket-cocina-item">${item.cantidad} x ${escapeHtml(item.producto_nombre || 'Producto')}</div>`)
    .join('');
...
export function imprimirPrueba() {
  ...
  const pedidoDePrueba = {
    ...
    total: 12000,
    nota: 'Fraccionar en bolsas de 1kg',
    items: [{ cantidad: 5, producto_nombre: 'Manzanilla (pack x10)', subtotal: 12000 }],
  };
