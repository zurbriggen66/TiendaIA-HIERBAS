 ...
 *
 * Se guarda POR DISPOSITIVO, no por cuenta: la compu del local puede ir en claro y el
 * celular del dueño en oscuro sin pisarse. La tienda del cliente no se ve afectada —
 * redeclara sus propios colores sobre `.cliente-container` (ver index.css).
 */
const CLAVE = 'hierbas_tema_admin';
...
// Oscuro por defecto: es como venía el panel, así que quien no toque nada no ve un
...
export const guardarTema = (tema) => {
  try {
    ...
    /* sin almacenamiento: igual se aplica en esta pestaña */
  }
  aplicarTema(tema);
};
